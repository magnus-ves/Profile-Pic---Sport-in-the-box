...
// ---------------------------------------------------------------------------
// Browser-based storage layer (replaces Electron IPC from the desktop app)
// ---------------------------------------------------------------------------
const bassengfoto = {
  settings: {
    load: () => JSON.parse(localStorage.getItem('bassengfoto:settings') || 'null'),
    save: (settings) => localStorage.setItem('bassengfoto:settings', JSON.stringify(settings))
  },
  athletes: {
    load: () => JSON.parse(localStorage.getItem('bassengfoto:athletes') || '[]'),
    save: (athletes) => localStorage.setItem('bassengfoto:athletes', JSON.stringify(athletes))
  },
  savedFileNames: new Set(JSON.parse(localStorage.getItem('bassengfoto:savedFiles') || '[]')),
  markFileSaved(fileName) {
    this.savedFileNames.add(fileName);
    localStorage.setItem('bassengfoto:savedFiles', JSON.stringify([...this.savedFileNames]));
  }
};
...
async function loadAll() {
  const stored = bassengfoto.settings.load() || {};
  state.settings = { ...DEFAULT_SETTINGS, ...stored, chromaKey: { ...DEFAULT_SETTINGS.chromaKey, ...(stored.chromaKey || {}) } };
  state.athletes = bassengfoto.athletes.load();
}

async function saveSettings() {
  bassengfoto.settings.save(state.settings);
}

async function saveAthletes() {
  bassengfoto.athletes.save(state.athletes);
}
...
function initCameraControls() {
  ...
  document.getElementById('btn-refresh-cameras').addEventListener('click', async () => {
    try {
      const permCheck = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
      permCheck.getTracks().forEach((t) => t.stop());
    } catch (err) {
      // ignore — listCameras() below will surface the error state
    }
    listCameras();
  });
}
...
// ---------------------------------------------------------------------------
// Crop overlay (drag + zoom) — mouse AND touch, for iPad
// ---------------------------------------------------------------------------
...
function initCropInteraction() {
  ...
  function onDragStart(e) {
    ...
    state.dragging = true;
    state.dragStart = pointFromEvent(e, box);
...
// ---------------------------------------------------------------------------
// Capture & save (browser download instead of writing to a shared folder)
// ---------------------------------------------------------------------------
...
async function captureAndSave() {
  ...
  const doSave = async () => {
    ...
    bassengfoto.markFileSaved(fileName);
    athlete.photographed = true;
    athlete.lastPhotoAt = new Date().toISOString();
    await saveAthletes();
    renderAthleteList();
    showSaveConfirmation(`Lastet ned: ${fileName}`, false);
  };

  if (bassengfoto.savedFileNames.has(fileName)) {
...
function openOverwriteModal(fileName, onConfirm) {
  ...
  document.getElementById('overwrite-message').textContent = `Et bilde med filnavnet "${fileName}" er allerede lastet ned tidligere i denne økten. Vil du laste ned på nytt?`;
  modal.classList.remove('hidden');
